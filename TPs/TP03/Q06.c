...
void inserirInicio(PilhaFlexivel* lista, Show s) {
...
void inserirFim(PilhaFlexivel* lista, Show s) {
    Node* novo = malloc(sizeof(Node));
    novo->data = s;
    novo->next = NULL;
...
void inserir(PilhaFlexivel* lista, Show s, int pos) {
    if (pos == 0) {
...
Show removerInicio(PilhaFlexivel* lista) {
    Node* temp = lista->head;
...
Show removerFim(PilhaFlexivel* lista) {
...
Show remover(PilhaFlexivel* lista, int pos) {
...
void mostrarPilha(PilhaFlexivel* lista) {
    // Copiar os elementos da pilha para um vetor
    Show* vetor = malloc(lista->size * sizeof(Show));
    Node* atual = lista->head;
    int i = 0;
...
int main() {
    ...
    int total = 0;
    PilhaFlexivel selecionados = {NULL, 0};
