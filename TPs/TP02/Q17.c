...
int compararTitulo(Show* a, Show* b, int* comparacoes) {
    (*comparacoes)++;
...
void heapify(Show* arr, int n, int i, int* comparacoes) {
    int maior = i;
    int esq = 2 * i + 1;
    int dir = 2 * i + 2;

    if (esq < n && compararTitulo(&arr[esq], &arr[maior], comparacoes) > 0) {
        maior = esq;
    }

    if (dir < n && compararTitulo(&arr[dir], &arr[maior], comparacoes) > 0) {
        maior = dir;
    }
    ...
    if (maior != i) {
        ...
        arr[i] = arr[maior];
        arr[maior] = temp;
        heapify(arr, n, maior, comparacoes);
    }
}

void heapSort(Show* arr, int n, int* comparacoes) {
    // Construir heap (reorganizar array)
    for (int i = n / 2 - 1; i >= 0; i--) {
        heapify(arr, n, i, comparacoes);
    }
...
int main() {
    ...
    heapSort(selecionados, qtdSelecionados, &comparacoes);
