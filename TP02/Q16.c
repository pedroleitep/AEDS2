...
int compararTitulo(Show* a, Show* b, int* comparacoes) {
    (*comparacoes)++;
    int cmp = strcasecmp(a->type, b->type);  // compara pelo tipo
    if (cmp != 0) return cmp;
    return strcasecmp(a->title, b->title);   // desempate pelo título
}


void insertionSort(Show* arr, int n, int* comparacoes) {
    for (int i = 1; i < n; i++) {
        Show temp = arr[i];
        int j = i - 1;

        while (j >= 0 && compararTitulo(&arr[j], &temp, comparacoes) > 0) {
            arr[j + 1] = arr[j];
            j--;
        }
...
int main() {
    ...
    insertionSort(selecionados, qtdSelecionados, &comparacoes);
