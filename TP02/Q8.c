...
int main() {
    ...
    // ShellSort por 'type' com desempate por 'title'
    for (int gap = qtdSelecionados / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < qtdSelecionados; i++) {
            Show temp = selecionados[i];
            int j = i;

            while (j >= gap) {
                comparacoes++;
                int cmpType = strcasecmp(temp.type, selecionados[j - gap].type);
                int cmpTitle = strcasecmp(temp.title, selecionados[j - gap].title);

                // Se temp.type < anterior, ou se type igual e temp.title < anterior.title, fazer troca
                if (cmpType < 0 || (cmpType == 0 && cmpTitle < 0)) {
                    selecionados[j] = selecionados[j - gap];
                    j -= gap;
                } else {
                    break;
                }
    ...
    FILE* log = fopen("843275_shellsort.txt", "w");
