...
void countingSort(Show* arr, int n, int exp, int* comparacoes) {
    ...
    for (int i = n - 1; i >= 0; i--) {
        int index = (arr[i].releaseYear / exp) % 10;
...
void insertionSortPorTitulo(Show* arr, int n, int* comparacoes) {
    for (int i = 1; i < n; i++) {
        Show key = arr[i];
        int j = i - 1;

        while (j >= 0 && arr[j].releaseYear == key.releaseYear &&
               strcasecmp(arr[j].title, key.title) > 0) {
            (*comparacoes)++;
            arr[j + 1] = arr[j];
            j--;
        }
...
void radixSort(Show* arr, int n, int* comparacoes) {
    int m = getMax(arr, n);

    for (int exp = 1; m / exp > 0; exp *= 10)
        countingSort(arr, n, exp, comparacoes);

    insertionSortPorTitulo(arr, n, comparacoes); // Desempate
}
...
int main() {
    ...
    radixSort(selecionados, qtdSelecionados, &comparacoes);
    ...
    FILE* log = fopen("843275_radixsort.txt", "w");
