...
void bubbleSort(Show* arr, int n, int* comparacoes) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - 1 - i; j++) {
            if (comparar(&arr[j], &arr[j + 1], comparacoes) > 0) {
                Show temp = arr[j];
                arr[j] = arr[j + 1];
                arr[j + 1] = temp;
            }
...
int main() {
    ...
    bubbleSort(selecionados, qtdSelecionados, &comparacoes);
    ...
    FILE* log = fopen("843275_bolha.txt", "w");
