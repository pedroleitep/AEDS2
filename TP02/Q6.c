...
void selectionSortRec(Show* arr, int n, int index, int* comparacoes) {
    if (index >= n - 1) return;
    ...
    if (menor != index) {
        swap(&arr[index], &arr[menor]);
    }

    selectionSortRec(arr, n, index + 1, comparacoes);
}
...
int main() {
    ...
    selectionSortRec(selecionados, qtdSelecionados, 0, &comparacoes);
    ...
    FILE* log = fopen("843275_selecaoRecursiva.txt", "w");
